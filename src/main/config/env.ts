export const env = {
  ...
  postgres_database: process.env.POSTGRES_DATABASE ?? 'class',
  ...
  email_from: process.env.EMAIL_FROM ?? 'guirozmesquita@gmail.com',
  email_fromDisplayName: process.env.EMAIL_FROMDISPLAYNAME ?? 'Guilherme',
  email_host: process.env.EMAIL_HOST ?? 'smtp.gmail.com',
  email_port: process.env.EMAIL_PORT ?? 465,
  email_ssl: handleBoolean(process.env.EMAIL_SSL),
  email_starttls: handleBoolean(process.env.EMAIL_STARTTLS),
  email_user: process.env.EMAIL_USER ?? 'guirozmesquita@gmail.com',
